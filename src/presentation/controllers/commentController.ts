...
export const createCommentController = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const comment = await createComment(postId, req.body.content, req.user.userId);
...
export const updateCommentController = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (comment.authorId !== req.user.userId && req.user.role !== "super") {
...
export const deleteCommentController = async (req: AuthRequest, res: Response) => {
  try {
    ...
    return res.status(400).json({ message: error instanceof Error ? error.message : "Failed to delete comment" });
  }
};
